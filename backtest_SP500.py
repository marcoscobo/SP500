...
df['Cum_Diff_close'] = df['Cum_Diff_close'] / df['Cum_Diff_close'][0]

# Calculate historical rentability
rent = df['Cum_Diff_close'][len(df) - 1] ** (1 / years_backtest)
print('Historical anual rentability: {:.2f}%\n'.format((rent - 1) * 100))

# Backtest (monthly)
total_amount = initial_contrib * df['Cum_Diff_close'][len(df) - 1]
for i in range(1, int(len(df) / 21)):
    total_amount += month_contrib * df['Cum_Diff_close'][len(df) - 1] / df['Cum_Diff_close'][21 * i]
...
for i in range(1, len(df)):
    total_amount += daily_contrib * df['Cum_Diff_close'][len(df) - 1] / df['Cum_Diff_close'][i]
total_contrib = initial_contrib + month_contrib * 12 * years_backtest
profit = total_amount - total_contrib
